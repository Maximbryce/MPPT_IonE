...
algorithmOutput step_algorithm(Hardware *const mpptHardware) {
    algorithmOutput algorithmResponse;

#if defined(IC)
    float curVoltage = mpptHardware->inputVoltage;
    float curCurrent = mpptHardware->inputCurrent;
...
#elif defined(PD)
    float curAverageVoltage = mpptHardware->inputVoltage;
    float curAverageCurrent = mpptHardware->get_average_input_current();
