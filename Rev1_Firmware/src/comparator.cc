...
COMP_HandleTypeDef batteryCurrentComp;
COMP_HandleTypeDef arrayVoltageComp;
COMP_HandleTypeDef arrayCurrentComp;
...
void init() {
    ...
    batteryCurrentComp.Init = defaultInit;
    batteryCurrentComp.Init.BlankingSrce = COMP_BLANKINGSRC_TIM8_OC5_COMP3;
    batteryCurrentComp.Init.InputMinus = COMP_INPUT_MINUS_DAC3_CH1;
    batteryCurrentComp.Instance = COMP3;

    // Set comparator 1 which is for voltage from the battery
    batteryVoltageComp.Init = defaultInit;
    batteryVoltageComp.Init.BlankingSrce = COMP_BLANKINGSRC_TIM8_OC5_COMP1;
    ...
    arrayCurrentComp.Init = defaultInit;
    arrayCurrentComp.Init.BlankingSrce = COMP_BLANKINGSRC_TIM8_OC5_COMP7;
    arrayCurrentComp.Init.InputMinus = COMP_INPUT_MINUS_DAC4_CH1;
    arrayCurrentComp.Instance = COMP7;

    // Set comparator 2 which is for voltage from the array
    arrayVoltageComp.Init = defaultInit;
    arrayVoltageComp.Init.BlankingSrce = COMP_BLANKINGSRC_TIM8_OC5_COMP2;
    arrayVoltageComp.Init.InputMinus = COMP_INPUT_MINUS_DAC3_CH2;
    arrayVoltageComp.Instance = COMP2;

    // Initialize the comparators
    bool status1 =
        HAL_COMP_Init(&batteryCurrentComp) == HAL_StatusTypeDef::HAL_OK;
    bool status2 =
        HAL_COMP_Init(&batteryVoltageComp) == HAL_StatusTypeDef::HAL_OK;
    bool status3 =
        HAL_COMP_Init(&arrayCurrentComp) == HAL_StatusTypeDef::HAL_OK;
    bool status4 =
...
COMP_HandleTypeDef *getComp3Handle() {
    return &batteryCurrentComp;
}

COMP_HandleTypeDef *getComp7Handle() {
    return &arrayCurrentComp;
}
